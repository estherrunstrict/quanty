...
Feature: **per-bot unrealized + realized P/L, surfaced on the dashboard.**
Branch: `feature/per-bot-pnl` (branched from `feature/adaptive-quant-framework`
at `58e5ef1` after WIP stashed as `stash@{0}`).

### Commits (most recent first)

| SHA | Subject |
|-----|---------|
| 14c9645 | chore(runner): check_and_run.sh refreshes realized-PnL after daily summary |
| 215fcfb | feat(dashboard): render Unrealized + Realized YTD per strategy card |
| e58d77a | feat(dashboard): expose per-bot unrealized + realized_ytd in API |
| a1394da | feat(pnl): add realized-PnL YTD aggregator + regression tests |
| 7654697 | chore: track active production scripts |
| d736ef9 | chore: ignore *_daily.csv price fixtures |
| ff5a71f | review: architecture audit of active code path |

### Files changed by category

**New**
- `scripts/aggregate_realized_pnl.py` — YTD realized-PnL aggregator, atomic write, configurable sources.
- `tests/test_aggregate_realized_pnl.py` — 5 regression tests (YTD window, missing state, nested path, malformed input, atomic replace).
- `strategy_results/realized_pl_2026.json` — gitignored output file (auto-generated by the aggregator).

**Modified**
- `dashboard_server.py` — after building strategies list, enrich each with `unrealized_profit`, `realized_profit_ytd`, `realized_trades` from the aggregator file. Hybrid VB legs (`kr`/`us`) get their own realized rollup.
- `dashboard.html` — relabel "P&L" / "Cum. P&L" → "Unrealized" across all cards; add conditional "Realized YTD" cell via `realizedCell()` helper. Hybrid VB splits KR / US legs.
- `check_and_run.sh` — invokes `aggregate_realized_pnl.py` after `portfolio_summary.py` each daily cycle.
- `.gitignore` — ignores `*_daily.csv` data fixtures and local `pnl-screenshots/` verification dir.

### Test status

`python3 -m pytest tests/` — **94 passed, 1 skipped**. 5 new tests cover the aggregator.

### Known limitations / follow-ups for reviewer

1. **The aggregator reads state-file trade-history, NOT the bots' stdout logs.** The 8 bot state files listed in `SOURCES` in `aggregate_realized_pnl.py` are the canonical source of truth. If any bot currently writes closed trades only to a log (not to its state file), the aggregator will return zero trades for that bot silently (state file absent → no error, zero). Spot-check on the server:
   ```
   for f in upbit_vb_strategy_state.json hybrid_vb_state.json korea_etf_momentum_state.json jd_strategy_state.json quant40_state.json claude_trading_bot_state.json modified_dual_momentum_state.json; do
     echo "--- $f ---"
     jq -r 'keys[], (.closed_trades // .trade_history // .kr.trade_history // .us.trade_history // []) | length' "$f" 2>/dev/null | head -4
   done
   ```
   Any bot that does NOT persist trades to its state file needs a small code change to do so. **That change was not made in this PR** because the scope was infrastructure + dashboard wiring.

2. **`strategy_results/realized_pl_2026.json` is gitignored** (the whole `strategy_results/` dir is ignored per project convention). This is correct — the file is runtime-generated. Deploy the aggregator script, not its output.

3. **`dashboard_server.py` reads `realized_pl_2026.json` by literal filename.** When 2027 rolls over, the reader will silently serve 2026 data (or nothing if the file is missing). Either: (a) name the dashboard-consumed file `realized_pl_current.json` and have the aggregator `os.replace` into it in addition to the dated file, or (b) make the reader glob-pick the latest year. Not urgent but schedule for December.

4. **No per-bot `*_result.json` schema change.** The prompt suggested adding `unrealized_profit` / `realized_profit_ytd` to each bot's result JSON. We skipped that layer — the dashboard server computes the split at serve time from existing fields + aggregator output. Simpler, one less place for drift. If an external consumer (Discord, etc.) needs the split in the result JSON itself, add it per-bot as a follow-up.

5. **JD Strategy and Quant40 share one render branch** (`s.id === 'quant40' || s.id === 'jd_strategy'`). The Realized YTD row appears in both — intended.
